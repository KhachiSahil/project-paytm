...
function Head({ value, userid }) {
  const [data, setData] = useState(null);

  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get(`http://localhost:3000/api/v1/user/userid?userid=${userid}`);
  ...
  return (
    <div className='flex flex-row justify-between'>
      ...
      <div className='text-cyan-700 flex mt-9 mr-10 '>
        ...
        <span className='mt-2 text-2xl text-cyan-700'>{data ? data.firstName +" " +data.lastName: 'Loading...'}</span>
      </div>
    </div>
  );
}
